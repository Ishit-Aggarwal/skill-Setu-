...
import { formatDateTime } from "./match";
...
export async function updateSlot(slotId, patch) {
  if (schedulingMode() === SHARED) {
    const result = await backendMutation(api.mentorship.updateSlot, { slotId, patch });
    /* Told the same way on both paths. This used to sit after the early
       return, so a mentor moving a session got nobody told about it in exactly
       the case where the students are on other devices — the only case where
       being told matters. */
    notifyBookedStudents(slotId, patch);
    return result;
  }
...
/**
 * Tells everyone holding a place that their session changed.
 *
 * The message names what actually moved rather than saying "details or
 * schedule have been updated", which leaves the reader to go and diff it
 * themselves. The mentor's name is looked up from the users table — an
 * officeHours row carries `facultyId`, not `mentorName`, so reading
 * `slot.mentorName` produced "Mentor" on every single notification.
 */
function notifyBookedStudents(slotId, patch = {}) {
  const slot = findOne("officeHours", (s) => s.id === slotId);
  const bookings = activeOnly(listBookingsForSlot(slotId));
  if (!bookings.length) return;

  const mentor = slot?.facultyId ? findOne("users", (u) => u.id === slot.facultyId) : null;
  const title = patch.title || slot?.title || "Office hours";

  const changes = [];
  if (patch.slot && patch.slot !== slot?.slot) changes.push(`it now starts at ${formatDateTime(patch.slot)}`);
  if (patch.mode && patch.mode !== slot?.mode) changes.push(`it is now ${patch.mode}`);
  if (patch.location && patch.location !== slot?.location) changes.push(`the venue is now ${patch.location}`);
  if (patch.meetingUrl && patch.meetingUrl !== slot?.meetingUrl) changes.push("the joining link has been added or changed");
  if (patch.durationMins && patch.durationMins !== slot?.durationMins) changes.push(`it now runs ${patch.durationMins} minutes`);

  const detail = changes.length ? changes.join(", ") : "some of its details have changed";

  bookings.forEach((booking) => {
    if (!booking.studentId) return;
    notifyStudent(
      booking.studentId,
      `Your mentoring session "${title}" was updated — ${detail}.`,
