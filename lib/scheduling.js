...
export async function publishSlot(user, form) {
  ...
  if (payload.mode === "Online" && payload.meetingUrl && !/^https?:\/\//i.test(payload.meetingUrl)) {
...
export async function updateSlot(slotId, patch) {
  ...
  return update("officeHours", slotId, patch);
}
...
/**
 * Slots the student can book. Locally that means every slot published by a
 * mentor at their own institution, which is the same rule the server applies.
 */
export async function loadAvailableSlots(user) {
  ...
  const mentors = listUsersByRole("academician").filter((m) => !institution || m.institution === institution);

  return mentors
    .flatMap((mentor) =>
      listOfficeHours(mentor.id).map((slot) => {
