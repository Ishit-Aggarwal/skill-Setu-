...
/** { text } stays; { inline_data: { mime_type, data } } becomes the REST API's camelCase. */
function normaliseParts(list) {
  return (Array.isArray(list) ? list : [])
    .map((p) => {
      if (!p) return null;
      if (typeof p.text === "string") return p.text ? { text: p.text } : null;
      const inline = p.inline_data || p.inlineData;
      const mimeType = inline?.mime_type || inline?.mimeType;
      if (inline?.data && mimeType) return { inlineData: { mimeType, data: inline.data } };
      return null;
    })
    .filter(Boolean);
}
 ...
 */
export async function generateJson({ prompt, schema, temperature = 0.6, attachments = [], parts: sourceParts = [], meta = null }) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new GeminiError(AI_NOT_CONFIGURED.body.error, { status: 503, code: "AI_NOT_CONFIGURED" });

  // `parts` are lib/docText.js output: text and inline_data, in source order,
  // each file introduced by its own header line so the model can cite it.
  const parts = [
    ...attachments.filter((a) => a?.data && a?.mimeType).map((a) => ({ inlineData: { mimeType: a.mimeType, data: a.data } })),
    ...normaliseParts(sourceParts),
    { text: prompt },
  ];
  const payload = {
  ...
  for (const model of models) {
    for (let attempt = 0; attempt <= BUSY_RETRY_DELAYS_MS.length; attempt += 1) {
      ...
      if (res.ok) {
        // Which model actually answered, for the response and the logs.
