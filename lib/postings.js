...
import { backfill, mergeRemote } from "./remoteSync";
...
export async function mirrorPosting(record) {
  ...
  try {
    await convexClient().mutation(api.internships.create, {
      ...
      ayushSystem: record.ayushSystem || undefined,
...
export async function syncRemotePostings() {
  ...
  /* Which rows may cross the demo wall is lib/demoIsolation.js's decision
     (sample catalogue rows and the other side's personas never do); which
     copy wins is lib/mergeRows.js's. View counters are per-device tallies,
     so the server's are dropped before the merge and this browser's stay. */
  const { changed, serverIds } = mergeRemote("internships", remote, {
    transform: ({ views, uniqueViews, ...row }) => row,
  });
  await backfill("internships", serverIds, mirrorPosting);
  return changed;
}
