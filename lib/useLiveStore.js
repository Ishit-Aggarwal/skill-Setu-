...
/**
 * A value that changes every `intervalMs`, for screens whose content is
 * decided by the clock as well as by the store — a test card that has to
 * turn from "joining open" to "joining closed" to "ended" while the person
 * is looking at it, with nothing else changing.
 */
...
/**
 * True while the viewport is narrower than `px` — for the few places where
 * CSS alone cannot adapt, such as the labels a chart library draws itself.
 * Starts false so the server and the first client render agree.
 */
export function useNarrowerThan(px = 640) {
  const [narrow, setNarrow] = useState(false);
  useEffect(() => {
    if (typeof window === "undefined" || !window.matchMedia) return undefined;
    const media = window.matchMedia(`(max-width: ${px - 1}px)`);
