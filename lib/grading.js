...
/**
 * A marked paper with the answer key taken out: each question keeps whether
 * the candidate got it right and what they chose, but loses the correct
 * options and the explanation. What an open-window candidate sees until the
 * window closes (lib/testWindow.js#canRevealAnswers).
 */
export function withholdAnswers(result) {
  if (!result || typeof result !== "object") return result;
  const breakdown = Array.isArray(result.breakdown)
    ? result.breakdown.map(({ correctIds, correctText, explanation, ...row }) => row)
    : result.breakdown;
  return { ...result, breakdown, answersWithheld: true };
}

/**
 * The grade printed on a certificate (when the host shows grades):
 * O ≥ 90, A+ ≥ 80, A ≥ 70, B+ ≥ 60, B ≥ 50, otherwise Pass.
 */
