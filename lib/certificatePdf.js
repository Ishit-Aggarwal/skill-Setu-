...
export async function renderCertificatePdf({ snapshot, logo, signature }) {
  ...
  if (logoImg && d.watermark?.enabled !== false) {
    ...
    const wm = fit(logoImg, W * share, H * share);
    page.drawImage(logoImg, { x: (W - wm.width) / 2, y: (H - wm.height) / 2, width: wm.width, height: wm.height, opacity: d.watermark?.opacity ?? 0.08 });
  }
