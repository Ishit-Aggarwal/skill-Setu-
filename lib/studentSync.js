...
import { currentAccount, isDemoMode } from "./store";
import { isDemoId } from "./demoIsolation";
...
export async function syncPortfolioForViewer(studentId) {
  if (!studentId || isDemoId(studentId) !== isDemoMode()) return 0;
  const remote = await pull("portfolio", api.portfolios.getForViewer, { studentId });
  if (!remote) return 0;
