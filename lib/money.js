...
export function formatStipend(posting) {
  ...
  // Zero is a real, deliberate answer — an unpaid role — not a missing figure.
...
/**
 * Whether a posting pays anything at all.
 *
 * A recruiter is allowed to enter 0, and a student should be able to tell at a
 * glance rather than by reading the figure — so every card carries a Paid or
 * an Unpaid badge. A posting with no number recorded counts as unpaid, which
 * is what its stipend line has always said.
 */
export function isPaidPosting(posting) {
  const amount = stipendAmountOf(posting);
  return amount != null && amount > 0;
}

export function payLabel(posting) {
  return isPaidPosting(posting) ? "Paid" : "Unpaid";
}
