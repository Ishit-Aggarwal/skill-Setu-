...
import { EXAM, SYNC } from "./settings";
import { CREDENTIAL_KINDS as SHARED_CREDENTIAL_KINDS } from "./credentials";
import { canEditMeeting, durationMinutes, hasEnded, isWindowTest, meetingMode, scheduledStartMs, testEndMs, testPhase } from "./testWindow";
import { durationString } from "./duration";
...
const DEMO_VERSION_KEY = "demoSeededV9";
...
/*
 * Parsed-collection cache.
 *
 * A screen reads the same collection dozens of times per render (a roster
 * asks for each student's assessment, applications and portfolio), and
 * every read used to re-parse the whole JSON string from localStorage. The
 * parsed array is kept here per storage key; it is replaced by this tab's own
 * writes and dropped when another tab writes the key (the `storage` event),
 * so a read is never stale. Callers get a fresh array (the rows themselves
 * are shared), so sorting or filtering a result never touches the cache.
 */
const parsed = new Map();

if (isBrowser()) {
  window.addEventListener("storage", (event) => {
    if (event.key == null) parsed.clear();
    else if (event.key.startsWith(NS)) parsed.delete(event.key);
  });
}

/** Forgets every parsed collection; for code that writes localStorage directly. */
...
/* Every write stamps `updatedAt`: it is what the merge rule in
   lib/mergeRows.js compares when this device and the shared database both
   hold a copy of the same row. */
export function insert(collection, doc) {
  const arr = read(collection);
  const record = { id: doc.id || genId(collection), updatedAt: new Date().toISOString(), ...doc };
  arr.push(record);
...
export function update(collection, id, patch) {
  ...
  arr[idx] = { ...arr[idx], updatedAt: new Date().toISOString(), ...patch };
  write(collection, arr);
...
/* ============================================================
   ...
   Screens keep reading this store synchronously. Behind every write
   below a mirror pushes the same row (same id) to Convex in the
   background, and behind every list read a throttled pull merges the
   server's rows back in and broadcasts a mutation. The sync modules
   are imported dynamically so this file never statically imports
   Convex — it is what they import.
   ============================================================ */

const TAB_SESSION_KEY = "ayusetu:tab_session";
const GLOBAL_SESSION_KEY = "ayusetu:session";

/** The id of the signed-in account on this device (tab first, then browser). */
export function currentAccountId() {
  if (!isBrowser()) return null;
  try {
    return window.sessionStorage.getItem(TAB_SESSION_KEY) || window.localStorage.getItem(GLOBAL_SESSION_KEY) || null;
  } catch {
    return null;
  }
}

/** The signed-in account's cached profile, or null. */
export function currentAccount() {
  const id = currentAccountId();
  return id ? findOne("users", (u) => u.id === id) : null;
}

const REMOTE_PULL_INTERVAL_MS = SYNC.PULL_INTERVAL_MS;
const pullClock = {};

/** One pull per collection per interval, however many times a screen reads. */
function pullThrottled(key, run, { force = false } = {}) {
  if (!isBrowser()) return;
  const now = Date.now();
  if (!force && now - (pullClock[key] || 0) < REMOTE_PULL_INTERVAL_MS) return;
  pullClock[key] = now;
  run();
}

/* True while the sample catalogue or the demo cohort is being written. Seed
   rows are per-device by design, and a stale session token from a previous
   sign-in must never be able to push them to the shared database. */
let seeding = false;

/**
 * Best-effort call into a sync module; never blocks or throws.
 *
 * Skipped while seeding, and whenever the account on this device and the
 * demo switch disagree (a demo persona outside demo mode, or a real account
 * inside it) — the server would attribute the row to whoever the token
 * belongs to, which is exactly the leak the demo wall exists to prevent.
 */
function via(load, run) {
  if (!isBrowser() || seeding) return;
  const id = currentAccountId();
  if (id && String(id).startsWith("demo-") !== isDemoMode()) return;
  load()
    .then(run)
    .catch(() => {
      /* offline, or no session — the next successful write or read re-syncs */
    });
}

const sync = {
  postings: () => import("./postings"),
  tests: () => import("./skillTestSync"),
  registrations: () => import("./registrationSync"),
  student: () => import("./studentSync"),
  mentoring: () => import("./mentoringSync"),
  collabs: () => import("./collabSync"),
  industry: () => import("./industrySync"),
  institution: () => import("./institutionSync"),
  directory: () => import("./directorySync"),
};

/** Pulls every collection a screen might read, ignoring the throttle. */
export function refreshFromServer() {
  Object.keys(pullClock).forEach((k) => {
    pullClock[k] = 0;
  });
}
...
export const TEST_WEIGHT = { Online: 1, Offline: 1.5, Hybrid: 1.5 };
...
function purgeLegacySamples() {
  ...
  try {
    ...
    ["seeded", "seededV2", "demoSeeded", "demoSeededV2", "demoSeededV3", "demoSeededV4", "demoSeededV5", "demoSeededV6", "demoSeededV7", "demoSeededV8"].forEach((key) =>
...
function pullRemotePostings({ force = false } = {}) {
  pullThrottled("postings", () => mirrorToServer((m) => m.syncRemotePostings()), { force });
}
...
export function patchInternship(internshipId, patch) {
  ...
  if (updated) mirrorToServer((m) => m.mirrorPostingPatch(internshipId, { ...patch, updatedAt: updated.updatedAt }));
  return updated;
}
...
function pullRemoteApplications() {
  pullThrottled("applications", () => via(sync.industry, (m) => m.syncRemoteApplications()));
}

export function listApplications() {
  pullRemoteApplications();
...
export function applyToInternship(internship, student, _ignoredMatch, note) {
  ...
  const record = insert("applications", {
    ...
    // Uploads live in file storage; a resume from before that move is still a
    // data URL on this device and opens here, but does not leave it.
  ...
  via(sync.industry, (m) => m.mirrorApplication(record));
  return record;
}
...
export function updateApplicationStatus(id, status, extra = {}) {
  ...
  if (updated) via(sync.industry, (m) => m.mirrorApplicationStatus(id, { ...patch, updatedAt: updated.updatedAt }));
...
export function updateApplicationRecruiterFields(id, patch) {
  const updated = update("applications", id, patch);
  if (updated) via(sync.industry, (m) => m.mirrorApplicationRecruiterFields(id, { ...patch, updatedAt: updated.updatedAt }));
  return updated;
}
...
export function setOfferStage(applicationId, offerStage, extra = {}) {
  ...
  const patch = { offerStage, offerSentAt, offerUpdatedAt: now, ...extra };
  const updated = update("applications", applicationId, patch);
  if (updated) via(sync.industry, (m) => m.mirrorApplicationRecruiterFields(applicationId, { ...patch, updatedAt: updated.updatedAt }));
  return updated;
}
...
export function listRecruiters(companyOwnerId) {
  pullThrottled("recruiters", () => via(sync.industry, (m) => m.syncRemoteRecruiters()));
  return findMany("recruiters", (r) => r.companyOwnerId === companyOwnerId);
}

export function addRecruiter(companyOwnerId, data) {
  const record = insert("recruiters", { companyOwnerId, addedAt: new Date().toISOString(), ...data });
  via(sync.industry, (m) => m.mirrorRecruiter(record));
  return record;
}

export function updateRecruiter(id, patch) {
  const updated = update("recruiters", id, patch);
  if (updated) via(sync.industry, (m) => m.mirrorRecruiterPatch(id, patch));
  return updated;
}

export function removeRecruiter(id) {
  remove("recruiters", id);
  via(sync.industry, (m) => m.mirrorRemoveRecruiter(id));
}

export function assignPostingToRecruiter(internshipId, recruiterId, recruiterName) {
  const updated = update("internships", internshipId, { recruiterId, recruiterName });
  if (updated) via(sync.industry, (m) => m.mirrorAssignPosting(internshipId, recruiterId, recruiterName));
  return updated;
}
...
export function listCompanyReviews(companyName) {
  pullThrottled("companyReviews", () => via(sync.industry, (m) => m.syncRemoteReviews()));
  return findMany("companyReviews", (r) => r.company === companyName);
}

export function addCompanyReview(companyName, review) {
  const record = insert("companyReviews", { company: companyName, authorId: currentAccountId() || undefined, createdAt: new Date().toISOString(), ...review });
  via(sync.industry, (m) => m.mirrorCompanyReview(record));
  return record;
}
...
function pullRemoteSkillTests({ force = false } = {}) {
  pullThrottled("skillTests", () => mirrorTests((m) => m.syncRemoteSkillTests()), { force });
}
...
export function patchSkillTest(testId, patch) {
  ...
  if (updated) mirrorTests((m) => m.mirrorSkillTestPatch(testId, { ...patch, updatedAt: updated.updatedAt }));
  return updated;
}

/* The timeline of a test — start, joining window, end — lives in
   lib/testWindow.js so the Convex functions read the same clock. These are
   the store's long-standing names for it. */
export function getScheduledTimestamp(test) {
  return scheduledStartMs(test);
}

/** Minutes a test runs for, read off its free-text duration ("60 mins"). */
export function testDurationMinutes(test) {
  return durationMinutes(test);
}

/** When the sitting is over — its start (pressed or scheduled) plus the test's own length. */
export function getTestEndTimestamp(test) {
  return testEndMs(test);
}

/** Certificates are a record of a completed test, so they wait for one. */
export function hasTestEnded(test) {
  return hasEnded(test);
}

/**
 * The absolute instant of a scheduled date and time, as this browser reads
 * them. Stored on the row so the server, which runs on UTC, never has to
 * guess the host's timezone.
 */
export function scheduledAtMsFor(scheduledAt, scheduledTime) {
  return scheduledStartMs({ scheduledAt, scheduledTime });
}

/**
 * The live meeting is optional: the exam room monitors an online sitting by
 * itself. Whether there is a meeting, and its link, may change until
 * EXAM.MEETING_LINK_LEAD_HOURS before the start (lib/testWindow.js), so a
 * registered candidate is never sent somewhere new at the last minute.
 */
function requireMeetingEditable(test) {
  if (test.mode === "Offline") throw new Error("Only online and hybrid tests have a meeting.");
  if (!canEditMeeting(test)) {
    throw new Error(`The meeting can no longer be changed — changes close ${EXAM.MEETING_LINK_LEAD_HOURS} hours before the test starts.`);
  }
}
...
export function setSkillTestMeetingLink(testId, meetingLink) {
  ...
  requireMeetingEditable(test);
  const link = String(meetingLink || "").trim();
  if (link && !/^https?:\/\/\S+$/i.test(link)) throw new Error("Enter a full link starting with https://");
  return patchSkillTest(testId, { meetingLink: link || null, meetingMode: link ? "live" : test.meetingMode || "none" });
}

/** "live" (the host runs a meeting alongside) or "none" (the exam room monitors on its own). */
export function setSkillTestMeetingMode(testId, mode) {
  const test = findOne("skillTests", (t) => t.id === testId);
  if (!test) throw new Error("Test not found.");
  requireMeetingEditable(test);
  const next = mode === "live" ? "live" : "none";
  return patchSkillTest(testId, { meetingMode: next, ...(next === "none" ? { meetingLink: null } : {}) });
}

export function startSkillTest(testId) {
  const test = findOne("skillTests", (t) => t.id === testId);
  if (!test) throw new Error("Test not found.");
  if (isWindowTest(test)) throw new Error("An open window opens by itself at its opening time.");
  if (test.mode !== "Offline" && meetingMode(test) === "live" && !test.meetingLink?.trim()) {
    throw new Error("You chose to run a live meeting for this test — add its link first, or switch the test to automatic monitoring.");
  }
  const phase = testPhase(test);
  if (phase === "ended") throw new Error("This test has already ended. Reschedule it to run another sitting.");
  if (phase === "open" || phase === "locked") throw new Error("This test is already in progress.");
  return patchSkillTest(testId, { startedAt: new Date().toISOString(), status: "In Progress" });
}

/**
 * Moves an open window. Before it opens anything may change; after, only the
 * close (the server refuses a shortening that would catch anyone mid-paper).
 * Registered candidates are told either way.
 */
export function rescheduleWindowTest(testId, { opensAtMs, closesAtMs, durationMinutes: minutes }) {
  const test = findOne("skillTests", (t) => t.id === testId);
  if (!test) throw new Error("Test not found.");
  const opens = new Date(opensAtMs);
  const pad = (n) => String(n).padStart(2, "0");
  const extended = Date.now() >= test.windowOpensAtMs && closesAtMs > test.windowClosesAtMs;
  const patch = {
    windowOpensAtMs: opensAtMs,
    windowClosesAtMs: closesAtMs,
    scheduledAtMs: opensAtMs,
    scheduledAt: `${opens.getFullYear()}-${pad(opens.getMonth() + 1)}-${pad(opens.getDate())}`,
    scheduledTime: `${pad(opens.getHours())}:${pad(opens.getMinutes())}`,
    ...(Number.isFinite(minutes) ? { durationMinutes: minutes, duration: durationString(minutes) } : {}),
  };
  const updated = patchSkillTest(testId, patch);
  const closeLabel = new Date(closesAtMs).toLocaleString("en-IN", { day: "numeric", month: "short", hour: "numeric", minute: "2-digit" });
  findMany("skillTestRegistrations", (r) => r.testId === testId && !r.cancelledAt).forEach((r) => {
    notifyStudent(
      r.userId,
      extended ? `"${test.title}": the window has been extended to ${closeLabel}.` : `"${test.title}": the window has changed. It now closes ${closeLabel}.`,
      test.hostName || "Test Host",
      { testId, kind: extended ? "test_window_extended" : "test_rescheduled", link: "/skill-assessment" }
    );
  });
  return updated;
}

export function rescheduleSkillTest(testId, { scheduledAt, scheduledTime, reportingTime, durationMinutes: minutes }) {
  const test = findOne("skillTests", (t) => t.id === testId);
  if (!test) throw new Error("Test not found.");

  if (test.mode !== "Online") {
    // Candidates have travel to arrange, so an upcoming on-site sitting is
    // frozen for its last day. One that has already run may be given a new
    // date: that is how a second sitting is scheduled.
    const scheduled = getScheduledTimestamp(test);
    if (scheduled && scheduled > Date.now() && scheduled - Date.now() < 24 * 60 * 60 * 1000) {
  ...
  const patch = {
    ...
    scheduledAtMs: scheduledAtMsFor(scheduledAt, scheduledTime || test.scheduledTime),
    // A new date is a new sitting: the earlier "Start Test", if any, no
    // longer describes when this one begins.
    startedAt: null,
    status: "Open",
  ...
  registrations.forEach((r) => {
    notifyStudent(
      ...
      test.hostName || "Test Host",
      { testId, kind: "test_rescheduled", link: "/skill-assessment" }
    );
  });
...
function pullRemoteRegistrations() {
  pullThrottled("registrations", () => via(sync.registrations, (m) => m.syncRemoteRegistrations()));
}

export function listRegistrationsForStudent(userId) {
  pullRemoteRegistrations();
  return findMany("skillTestRegistrations", (r) => r.userId === userId);
}

/** Every registration a host can see — the roster behind each test card. */
export function listRegistrationsForHost() {
  pullRemoteRegistrations();
  return all("skillTestRegistrations");
}

export function registerForSkillTest(testId, userId, info) {
  const existing = getRegistration(testId, userId);
  if (existing) return existing;
  const record = insert("skillTestRegistrations", {
    testId,
    userId,
    ...info,
  ...
  via(sync.registrations, (m) => m.mirrorRegistration(record));
  return record;
}

function patchRegistration(id, patch) {
  const updated = update("skillTestRegistrations", id, patch);
  if (updated) via(sync.registrations, (m) => m.mirrorRegistrationPatch(id, patch));
  return updated;
}
...
/* Attempts the server graded without this browser (a closed window, the
   abandonment sweep) arrive on this pull. */
function pullRemoteAttempts() {
  pullThrottled("attempts", () => mirrorTests((m) => m.syncRemoteAttempts()));
}
...
export function recordAssessmentResult(studentId, testId, domain, scorePct, weight = 1, missed = false, extra = {}) {
  ...
  if (reg) patchRegistration(reg.id, { missedRecorded: true });
...
export function confirmOfflineAttendance(studentId, testId) {
  const reg = getRegistration(testId, studentId);
  if (reg) return patchRegistration(reg.id, { attended: true, attendedAt: new Date().toISOString() });
  return reg;
}
...
export function checkAndRecordMissedTests(studentId) {
  ...
  regs.forEach((reg) => {
    ...
    if (!test || test.cancelledAt || reg.cancelledAt) return;
    // A window is only missed once it has closed, not a day after it opened.
    const scheduled = isWindowTest(test) ? testEndMs(test) : getScheduledTimestamp(test);
    if (!scheduled) return;
...
/**
 * A portfolio, from this device's store.
 *
 * The signed-in student's own is pulled from the shared database on the
 * usual throttle; somebody else's (a recruiter opening a candidate, a mentor
 * opening a mentee) is fetched through the server's viewer query, which
 * applies the student's visibility settings before answering.
 */
export function getPortfolio(studentId) {
  if (studentId) {
    const me = currentAccountId();
    if (studentId === me) pullThrottled("portfolio", () => via(sync.student, (m) => m.syncRemotePortfolio()));
    else pullThrottled("portfolios:viewer", () => via(sync.student, (m) => m.syncPortfoliosForReviewer()));
  }
...
export function savePortfolio(studentId, patch) {
  ...
  if (existing) {
    const updated = update("portfolios", existing.id, patch);
    if (updated) via(sync.student, (m) => m.mirrorPortfolio(updated, { ...patch, updatedAt: updated.updatedAt }));
    return updated;
  }
  const record = insert("portfolios", { studentId, ...patch });
  via(sync.student, (m) => m.mirrorPortfolio(record));
  return record;
}
...
export const CREDENTIAL_KINDS = SHARED_CREDENTIAL_KINDS;
...
export function issueCredential(issuer, student, data = {}) {
  ...
  const record = insert("credentials", {
    ...
    // No code until the server assigns its unique one: a placeholder here
    // could be printed or shared before the mirror answers, and would never verify.
    verifyCode: null,
  ...
  /* The server is authoritative for the certificate number and the
     verification code: the row above holds this device's placeholders until
     the mirror answers, then is patched with the values `/verify` will
     recognise from anywhere. The recipient's inbox row is written by the
     server in the same call; the local one below is for this device only. */
  via(sync.tests, async (m) => {
    const result = await m.mirrorManualCredential(record);
    if (result?.certificateNo) {
      update("credentials", record.id, { certificateNo: result.certificateNo, verifyCode: result.verifyCode, issuedAt: result.issuedAt || record.issuedAt });
    }
  });
  notifyStudent(
    student.id,
    ...
    issuerName,
    { credentialId: record.id, localOnly: true }
  );
  return record;
}
...
/* Certificates issued on the server (automatic ones) and by other partners
   (manual ones) arrive on the next read. */
function pullRemoteCredentials() {
  pullThrottled("credentials", () => mirrorTests((m) => m.syncRemoteCredentials()));
}
...
export function revokeCredential(id) {
  const updated = update("credentials", id, { revokedAt: new Date().toISOString() });
  if (updated) via(sync.tests, (m) => m.mirrorRevokeCredential(id));
  return updated;
}

/**
 * The student's display choices on one of their certificates, after the
 * server (certificates.setDisplay) has accepted them. Only these two fields
 * ever change on the student's side.
 */
export function applyCredentialDisplay(id, { showOnProfile, featured }) {
  const patch = {};
  if (showOnProfile !== undefined) {
    patch.showOnProfile = showOnProfile;
    if (!showOnProfile) patch.featured = false;
  }
  if (featured !== undefined) {
    patch.featured = featured;
    if (featured) patch.showOnProfile = true;
  }
  return update("credentials", id, patch);
}
...
function pullRemoteSaved() {
  pullThrottled("saved", () => via(sync.student, (m) => m.syncRemoteSaved()));
}
...
export function toggleSavedInternship(studentId, internshipId) {
  ...
  if (existing) {
    ...
    via(sync.student, (m) => m.mirrorSavedInternship(existing, false));
    return false;
  }
  const record = insert("savedInternships", { studentId, internshipId, savedAt: new Date().toISOString() });
  via(sync.student, (m) => m.mirrorSavedInternship(record, true));
  return true;
}
...
export function toggleSavedMentorship(studentId, slot) {
  ...
  if (existing) {
    ...
    via(sync.student, (m) => m.mirrorSavedMentorship(existing, false));
    return false;
  }
  const record = insert("savedMentorships", {
    studentId,
    slotId,
  ...
  via(sync.student, (m) => m.mirrorSavedMentorship(record, true));
  return true;
}

/* ---------- Saved talent-pool searches ----------
   A recruiter's named filter sets. They were written straight into the
   store from the talent pool page; going through here is what mirrors them. */

export function listSavedSearches(ownerId) {
  pullRemoteSaved();
  return findMany("savedSearches", (s) => s.ownerId === ownerId);
}

export function saveSearch(ownerId, name, filters) {
  const record = insert("savedSearches", { ownerId, name, filters, savedAt: new Date().toISOString() });
  via(sync.student, (m) => m.mirrorSavedSearch(record));
  return record;
}

export function removeSavedSearch(id) {
  remove("savedSearches", id);
  via(sync.student, (m) => m.mirrorDeleteSavedSearch(id));
}
...
function notifyProgrammeRegistrants(programme, reason) {
  listProgramRegistrations(programme.id).forEach((reg) => {
    if (reg.userId) {
      notifyStudent(
        reg.userId,
        `"${programme.title}" has been cancelled${reason ? ` — ${reason}` : ""}. You have been removed from the programme; no action is needed.`,
        programme.organiser || "Programme host"
      );
    }
...
export function runProgrammeLinkEscalation() {
  ...
  read("programs").forEach((programme) => {
    ...
    listProgramRegistrations(programme.id).forEach((reg) => {
      ...
      notifyStudent(
        reg.userId,
        `"${programme.title}" has moved back by one day while the host confirms the joining link. Your place is unchanged.`,
        programme.organiser || "Programme host"
      );
    });
  });
...
export function setCollabResponse(collabId, response) {
  ...
  via(sync.collabs, (m) => m.mirrorCollabResponse(collabId, response));
  if (existing) return update("collabResponses", existing.id, { response });
  return insert("collabResponses", { collabId, ownerId: currentAccountId() || undefined, response });
}

function pullRemoteCollabs() {
  pullThrottled("collabs", () => via(sync.collabs, (m) => m.syncRemoteCollabs()));
}
...
export function createCollabListing(ownerId, ownerName, data) {
  const record = insert("collabListings", {
    ownerId,
    ownerName,
    status: "Open",
  ...
  via(sync.collabs, (m) => m.mirrorCollabListing(record));
  return record;
}

export function updateCollabListing(id, patch) {
  const updated = update("collabListings", id, patch);
  if (updated) via(sync.collabs, (m) => m.mirrorCollabListingPatch(id, patch));
  return updated;
}
...
export function expressCollabInterest(listingId, user, message) {
  ...
  if (existing) return existing;
  const record = insert("collabInterests", {
    listingId,
    userId: user.id,
    name: user.name,
  ...
  via(sync.collabs, (m) => m.mirrorCollabInterest(record));
  return record;
}

export function setCollabInterestStatus(interestId, status) {
  const updated = update("collabInterests", interestId, { status });
  if (updated) via(sync.collabs, (m) => m.mirrorCollabInterestStatus(interestId, status));
  return updated;
}
...
export function postCollabMessage(collabId, author, body) {
  const record = insert("collabMessages", { collabId, author, body, at: new Date().toISOString() });
  via(sync.collabs, (m) => m.mirrorCollabMessage(record));
  return record;
}
...
export function addCollabMilestone(collabId, data) {
  ...
  const existing = title
    ...
    : null;
  if (existing) return existing;
  const record = insert("collabMilestones", { collabId, done: false, createdAt: new Date().toISOString(), ...data });
  via(sync.collabs, (m) => m.mirrorCollabMilestone(record));
  return record;
}
...
export function toggleCollabMilestone(id) {
  ...
  if (!m) return null;
  const updated = update("collabMilestones", id, { done: !m.done, completedAt: !m.done ? new Date().toISOString() : null });
  if (updated) via(sync.collabs, (mod) => mod.mirrorCollabMilestoneToggle(id, updated.done));
  return updated;
}
...
export function addCollabFile(collabId, file) {
  const record = insert("collabFiles", { collabId, uploadedAt: new Date().toISOString(), ...file });
  via(sync.collabs, (m) => m.mirrorCollabFile(record));
  return record;
}

export function removeCollabFile(id) {
  remove("collabFiles", id);
  via(sync.collabs, (m) => m.mirrorRemoveCollabFile(id));
}

export function listResearchOutputs(facultyId) {
  pullThrottled("research", () => via(sync.collabs, (m) => m.syncRemoteResearch()));
  return findMany("researchOutputs", (o) => o.facultyId === facultyId);
}

export function addResearchOutput(facultyId, data) {
  const record = insert("researchOutputs", { facultyId, addedAt: new Date().toISOString(), ...data });
  via(sync.collabs, (m) => m.mirrorResearchOutput(record));
  return record;
}

export function updateResearchOutput(id, patch) {
  const stamped = { ...patch, updatedAt: new Date().toISOString() };
  const updated = update("researchOutputs", id, stamped);
  if (updated) via(sync.collabs, (m) => m.mirrorResearchOutputPatch(id, stamped));
  return updated;
}

export function removeResearchOutput(id) {
  remove("researchOutputs", id);
  via(sync.collabs, (m) => m.mirrorRemoveResearchOutput(id));
}
...
function pullRemoteMentoring() {
  pullThrottled("mentoring", () => via(sync.mentoring, (m) => m.syncRemoteMentoring()));
}
...
/** The mentors who have taken a student on — the student's side of `advisees`. */
export function listMentorsForStudent(studentId) {
  pullRemoteMentoring();
  return findMany("advisees", (a) => a.studentId === studentId);
}
...
export function addAdvisee(facultyId, studentId) {
  ...
  const record = insert("advisees", { facultyId, studentId, since: new Date().toISOString() });
  via(sync.mentoring, (m) => m.mirrorAdvisee(record));
  return record;
}
...
export function removeAdvisee(facultyId, studentId) {
  ...
  if (rec) {
    remove("advisees", rec.id);
    via(sync.mentoring, (m) => m.mirrorRemoveAdvisee(studentId));
  }
}
...
export function saveMentorNote(facultyId, studentId, patch) {
  ...
  const stamped = { ...patch, updatedAt: new Date().toISOString() };
  const record = existing ? update("mentorNotes", existing.id, stamped) : insert("mentorNotes", { facultyId, studentId, ...stamped });
  if (record) via(sync.mentoring, (m) => m.mirrorMentorNote(record));
  return record;
}
...
export function recommendPostingToStudent(faculty, studentId, internship) {
  ...
  notifyStudent(studentId, `${faculty.name} recommended you apply to "${internship.title}" at ${internship.company}.`, faculty.name);
  return saved;
}
...
export function requestMentorship(student, faculty, message) {
  ...
  // The server writes the faculty member's inbox row itself when the request
  // is mirrored; this one is for this device only.
  notifyStudent(
    faculty.id,
    `${student.name}${student.department ? ` (${student.department})` : ""} has asked you to mentor them.${message ? ` "${message.trim()}"` : ""}`,
    student.name,
    { localOnly: true }
  );
  via(sync.mentoring, (m) => m.mirrorMentorshipRequest(record));
...
export function setMentorshipRequestStatus(requestId, status) {
  ...
  if (record) via(sync.mentoring, (m) => m.mirrorMentorshipRequestStatus(requestId, status));
  if (record?.studentId) {
    notifyStudent(
      record.studentId,
      status === "Accepted"
      ...
      record.facultyName || "Your institution",
      { localOnly: true }
    );
  }
  return record;
...
/**
 * Institution rows are owned by the institution ACCOUNT (`institutionId`),
 * not by the institute's typed name — see lib/institutionKey.js. On this
 * device the id comes from the signed-in institution account, or from a
 * cached institution account whose name matches; `instituteName` stays on
 * every row for display and for the screens that still key by it.
 */
function institutionIdFor(instituteName) {
  const me = currentAccount();
  if (me?.role === "institution" && (!instituteName || !me.instituteName || me.instituteName === instituteName)) return me.id;
  const account = findOne("users", (u) => u.role === "institution" && u.instituteName === instituteName);
  return account?.id || undefined;
}

function pullRemoteInstitution() {
  pullThrottled("institution", () => via(sync.institution, (m) => m.syncRemoteInstitution()));
}

/** Local write + mirror for a row the institution owns. */
function insertInstitutionRow(table, instituteName, doc) {
  const record = insert(table, { instituteName, institutionId: institutionIdFor(instituteName), ...doc });
  via(sync.institution, (m) => m.mirrorInstitutionRow(table, record));
  return record;
}

function updateInstitutionRow(table, id, patch) {
  const updated = update(table, id, patch);
  if (updated) via(sync.institution, (m) => m.mirrorInstitutionPatch(table, id, { ...patch, updatedAt: updated.updatedAt }));
  return updated;
}

function removeInstitutionRow(table, id) {
  remove(table, id);
  via(sync.institution, (m) => m.mirrorRemoveInstitutionRow(table, id));
}

export function getInstitutionProfile(instituteName) {
  pullRemoteInstitution();
  pullThrottled("institutionProfiles", () => via(sync.directory, (m) => m.syncRemoteInstitutionProfiles()));
...
export function saveInstitutionProfile(instituteName, patch) {
  const existing = findOne("institutionProfiles", (p) => p.instituteName === instituteName);
  if (existing) return updateInstitutionRow("institutionProfiles", existing.id, patch);
  return insertInstitutionRow("institutionProfiles", instituteName, patch);
}

export function listInstitutionStudents(instituteName) {
  pullThrottled("roster", () => via(sync.institution, (m) => m.syncRemoteRoster()));
...
/**
 * A student added to the roster before they have signed up. The row is a
 * real account on the shared database (no password, `invited`), so the
 * student can claim it by registering with the same email from any device;
 * this device keeps a copy so the roster renders at once.
 */
export function createStudentRecord(instituteName, data) {
  ...
  const record = insert("users", {
    ...
    institutionId: institutionIdFor(instituteName),
    passwordHash: null,
    invited: true,
    emailVerified: false,
    createdAt: new Date().toISOString(),
  ...
  via(sync.institution, async (m) => {
    const result = await m.mirrorInvitedStudent(record);
    // The server may already know this person (they registered themselves);
    // in that case its account replaces the placeholder under the real id.
    if (result?.ok && result.id && result.id !== record.id) {
      remove("users", record.id);
      if (result.user && !findOne("users", (u) => u.id === result.id)) insert("users", { ...result.user, id: result.id });
    }
  });
  return record;
}
...
export function addInstitutionAdmin(instituteName, data) {
  return insertInstitutionRow("institutionAdmins", instituteName, { addedAt: new Date().toISOString(), status: "Active", ...data });
}

export function updateInstitutionAdmin(id, patch) {
  return updateInstitutionRow("institutionAdmins", id, patch);
}

export function removeInstitutionAdmin(id) {
  removeInstitutionRow("institutionAdmins", id);
}

export function logActivity(scope, actor, action, detail = "") {
  return insertInstitutionRow("activityLog", scope, { scope, actor, action, detail, at: new Date().toISOString() });
}
...
export function createMou(instituteName, data) {
  return insertInstitutionRow("mous", instituteName, {
    createdAt: new Date().toISOString(),
...
export function updateMou(id, patch) {
  return updateInstitutionRow("mous", id, patch);
}

export function addMouTimelineEvent(id, label) {
  const mou = findOne("mous", (m) => m.id === id);
  if (!mou) return null;
  return updateInstitutionRow("mous", id, { timeline: [...(mou.timeline || []), { label, at: new Date().toISOString() }] });
}

export function deleteMou(id) {
  removeInstitutionRow("mous", id);
}
...
export function createAnnouncement(instituteName, data) {
  return insertInstitutionRow("announcements", instituteName, { postedAt: new Date().toISOString(), ...data });
}

export function updateAnnouncement(id, patch) {
  return updateInstitutionRow("announcements", id, patch);
}

export function deleteAnnouncement(id) {
  removeInstitutionRow("announcements", id);
}
...
export function addInstitutionDoc(instituteName, data) {
  return insertInstitutionRow("institutionDocs", instituteName, { uploadedAt: new Date().toISOString(), ...data });
}

export function removeInstitutionDoc(id) {
  removeInstitutionRow("institutionDocs", id);
}
...
export function createDrive(instituteName, data) {
  ...
  if (duplicate) return updateInstitutionRow("drives", duplicate.id, data) || duplicate;
  return insertInstitutionRow("drives", instituteName, { status: "Scheduled", createdAt: new Date().toISOString(), ...data });
}

export function updateDrive(id, patch) {
  return updateInstitutionRow("drives", id, patch);
}

/** The server cascades to the drive's invites and tags the same way. */
export function deleteDrive(id) {
  removeInstitutionRow("drives", id);
...
function insertDriveChild(table, doc) {
  const record = insert(table, doc);
  via(sync.institution, (m) => m.mirrorDriveChild(table, record));
  return record;
}
...
export function inviteCompanyToDrive(driveId, data) {
  ...
  if (existing) return existing;
  return insertDriveChild("driveInvites", { driveId, rsvp: "Invited", invitedAt: new Date().toISOString(), ...data });
}

export function setDriveInviteRsvp(inviteId, rsvp, extra = {}) {
  const updated = update("driveInvites", inviteId, { rsvp, rsvpAt: new Date().toISOString(), ...extra });
  if (updated) via(sync.institution, (m) => m.mirrorDriveChild("driveInvites", updated));
  return updated;
}

export function removeDriveInvite(inviteId) {
  remove("driveInvites", inviteId);
  via(sync.institution, (m) => m.mirrorRemoveDriveChild("driveInvites", inviteId));
}
...
/** The drives a student has been tagged for — read from the student's side. */
export function listDriveEligibilityForStudent(studentId) {
  pullRemoteInstitution();
  return findMany("driveEligibility", (e) => e.studentId === studentId);
}
...
export function tagStudentsForDrive(driveId, studentIds) {
  ...
  studentIds.forEach((studentId) => {
    ...
    insertDriveChild("driveEligibility", { driveId, studentId, taggedAt: new Date().toISOString() });
    added += 1;
  });
  return added;
}
...
export function untagStudentFromDrive(driveId, studentId) {
  ...
  if (rec) {
    remove("driveEligibility", rec.id);
    via(sync.institution, (m) => m.mirrorRemoveDriveChild("driveEligibility", rec.id));
  }
}
...
export function notifyStudent(studentId, message, from, meta = {}) {
  ...
  const { localOnly, ...rest } = meta;
  const record = insert("studentNotifications", {
    studentId,
    message,
    from,
    ...rest,
  ...
  // `localOnly` marks a row the server writes itself in the same operation
  // (a mentorship request, an issued certificate) — mirroring it too would
  // put two copies in the recipient's inbox.
  if (!localOnly) via(sync.student, (m) => m.mirrorNotification(record));
  return record;
}
...
export function notifyStudents(instituteName, studentIds, message, from) {
  ...
  const rows = studentIds.map((studentId) =>
    notifyStudent(studentId, message, from, { batchId: batch.id, sentAt: batch.sentAt, localOnly: true })
  );
  via(sync.student, (m) => m.mirrorNotifyBatch(batch, studentIds, rows.map((r) => r?.id)));
  return batch;
}
...
function pullRemoteNotifications() {
  pullThrottled("inbox", () => via(sync.student, (m) => m.syncRemoteNotifications()));
}
...
/** Marks inbox rows read on this device and on the shared database. */
export function markNotificationsRead(ids) {
  const list = (Array.isArray(ids) ? ids : [ids]).filter(Boolean);
  const readAt = new Date().toISOString();
  const changed = list.filter((id) => update("studentNotifications", id, { read: true, readAt }));
  if (changed.length) via(sync.student, (m) => m.mirrorMarkRead(changed, readAt));
...
export function upsertPlacementHistory(instituteName, record) {
  ...
  if (existing) return updateInstitutionRow("placementHistory", existing.id, record);
  return insertInstitutionRow("placementHistory", instituteName, record);
}

export function updatePlacementHistory(id, patch) {
  return updateInstitutionRow("placementHistory", id, patch);
}

export function removePlacementHistory(id) {
  removeInstitutionRow("placementHistory", id);
}
...
export function listUsersByRole(role) {
  pullThrottled(`users:${role}`, () => via(sync.directory, (m) => m.syncRemoteUsersByRole(role)));
...
function seedDemoCohort() {
  ...
  try {
    ...
    ["demoSeeded", "demoSeededV2", "demoSeededV3", "demoSeededV4", "demoSeededV5", "demoSeededV6", "demoSeededV7", "demoSeededV8"].forEach((key) => window.localStorage.removeItem(nsKey(key)));
  } catch {
    // ignore
  }
...
/**
 * Puts the tour back to its seeded state on this device: every collection in
 * the demo namespace is dropped, the seed flags with them, and the sample
 * catalogue and cohort are written again exactly as on first entry. Rows the
 * personas mirrored to the shared database are removed by the server in the
 * same operation (see lib/demoReset.js); nothing outside the demo namespace
 * is touched.
 */
export function resetLocalDemoData() {
  ...
  for (let i = 0; i < window.localStorage.length; i += 1) {
    const key = window.localStorage.key(i);
    if (key && key.startsWith(NS + DEMO_NS)) keys.push(key);
  }
  ...
  setDemoMode(true);
  ensureDemoData();
  broadcastMutation("users", "BATCH", { count: 0 });
}
