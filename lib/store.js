...
export function updateApplicationStatus(id, status) {
  ...
  const updated = update("applications", id, { status, statusHistory });
  if (existing?.studentId) {
    insert("studentNotifications", {
      studentId: existing.studentId,
      message: `Your application for "${existing.internshipTitle || "Internship"}" at ${existing.company || "Company"} was moved to ${status}.`,
      from: existing.company || "Recruiter",
