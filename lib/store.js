...
export function listPlacementHistory(instituteName) {
  return findMany("placementHistory", (h) => h.instituteName === instituteName).sort((a, b) => a.batch - b.batch);
}

export function upsertPlacementHistory(instituteName, record) {
  const existing = findOne(
    "placementHistory",
    (h) => h.instituteName === instituteName && h.batch === record.batch && h.department === record.department
  );
