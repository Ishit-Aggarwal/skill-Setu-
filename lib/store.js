...
/* ============================================================
   ...
   SIH26044 is administered by the Ministry of Ayush, but the
   portal it asks for is a general academia-industry platform.
   So the sample employers, institutions, postings and tests
   below are a balanced cross-industry mix — software, data,
   manufacturing, finance, design, logistics, healthcare and
   AYUSH — with AYUSH weighted like any other vertical rather
   than as the default. Seeded records use ownerId: "seed" so
   they never show up as "mine" for a fresh account.

   Postings carry real eligibility criteria (minimum skill score
   and eligible departments), because "only show roles I'm
   eligible for" can only filter on criteria that exist.
   ============================================================ */
...
function buildDemoCohort() {
  ...
  for (let i = 0; i < 60; i += 1) {
    ...
    // Panchakarma score (or a BAMS student a programming one) is what made the
...
export function ensureDemoData() {
  if (!isBrowser()) return;
  ensureSeeded();
  if (window.localStorage.getItem(NS + "demoSeededV4")) return;
  ...
  try {
    ...
    ["demoSeeded", "demoSeededV2", "demoSeededV3"].forEach((key) => window.localStorage.removeItem(NS + key));
  } catch {
    // ignore
  }
  ...
  window.localStorage.setItem(NS + "demoSeededV4", "1");
}
