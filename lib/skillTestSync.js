...
import { acceptRemoteRow } from "./demoIsolation";
...
export async function publishSkillTest(record, questions) {
  ...
  const {
    id, title, domain, ayushSystem, hostName, mode, duration, price, scheduledAt, scheduledTime, scheduledAtMs, reportingTime, venue,
    description, prerequisites, certification, rules, documentsRequired, meetingLink, meetingMode, status, postedAt,
    proctored, autoDisqualifyAfter, issueCertificate, minCertificateScore, violationPenalty, faceMonitoring, monitorViolationLimit, samplePapers,
    durationMinutes, scheduleType, windowOpensAtMs, windowClosesAtMs, shuffle, poolSize, audience, communityId, pinInCommunity,
  } = record;
  return await convexClient().mutation(api.skillTests.publishTest, {
    ...
    scheduledAtMs: Number.isFinite(scheduledAtMs) ? scheduledAtMs : undefined,
    ...
    violationPenalty: violationPenalty ?? null,
    faceMonitoring: faceMonitoring ?? undefined,
    monitorViolationLimit: monitorViolationLimit ?? null,
    samplePapers: Array.isArray(samplePapers) ? samplePapers : undefined,
    updatedAt: record.updatedAt || undefined,
    durationMinutes: Number.isFinite(durationMinutes) ? durationMinutes : undefined,
    scheduleType: scheduleType || undefined,
    windowOpensAtMs: Number.isFinite(windowOpensAtMs) ? windowOpensAtMs : undefined,
    windowClosesAtMs: Number.isFinite(windowClosesAtMs) ? windowClosesAtMs : undefined,
    shuffle: shuffle ?? undefined,
    poolSize: poolSize ?? null,
    audience: audience || undefined,
    communityId: communityId || undefined,
    pinInCommunity: pinInCommunity ? true : undefined,
...
export async function syncRemoteSkillTests() {
  ...
  let remote;
  try {
    // The session decides which community-only tests come back.
    remote = await convexClient().query(api.skillTests.listAll, getSessionToken() ? { sessionToken: getSessionToken() } : {});
  } catch (error) {
  ...
  // never seen has no paper and is not a test anyone can sit. A community
  // test this account can no longer see (removed from the community) is
  // pruned from the device.
  const account = currentAccount();
  return mergeRemote("skillTests", remote, {
    prune: (row) => row.audience === "community" && row.ownerId !== account?.id,
  }).changed;
}
...
/**
 * A student's graded attempts. Most arrive through the exam room itself,
 * but an attempt the server closed on its own — a window that was shut, a
 * sitting the abandonment sweep failed — has no browser to write it, so it
 * is pulled here. Server rows carry no client id: they are matched to the
 * local row by test, and given one when they are new.
 */
export async function syncRemoteAttempts() {
  if (!canSync()) return 0;
  const account = currentAccount();
  if (!account || account.role !== "student") return 0;
  const rows = await pull("attempts", api.skillTests.attemptsForStudent);
  if (!Array.isArray(rows)) return 0;
  const local = all("assessmentAttempts");
  const byTest = new Map(local.filter((a) => a.studentId === account.id).map((a) => [a.testId, a]));
  let changed = 0;
  rows.forEach(({ _id, _creationTime, ...row }) => {
    if (!acceptRemoteRow("assessmentAttempts", row, { demoMode: isDemoMode() })) return;
    const existing = byTest.get(row.testId);
    if (existing) {
      // The server's verdict wins; the local row keeps its id and anything the
      // exam room stored that the server does not (its own timing, say).
      const before = JSON.stringify(existing);
      Object.assign(existing, row, { id: existing.id });
      if (JSON.stringify(existing) !== before) changed += 1;
    } else {
      local.push({ ...row, id: `assessmentAttempts_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}` });
      changed += 1;
    }
  });
  if (changed) {
    saveAll("assessmentAttempts", local);
    broadcastMutation("assessmentAttempts", "BATCH", { count: changed });
  }
  return changed;
}
